...
def _mine_res4d(
    subj, sess, work_deriv, res_path, mask_path, model_name,
    task_name, weight_maps, log_dir
):
    """Title."""
    #
    do_dot = process.DoDot(res_path, mask_path)
    do_dot.parallel_dot(weight_maps, subj, sess, log_dir)
    do_dot.label_vol()
    ...
    out_path = os.path.join(
        out_dir, f"df_dot-product_model-{model_name}_task-{task_name}.csv"
    )
    do_dot.df_prod.to_csv(out_path, index=False)


def wf_emorep(
    subj, sess_list, mask_name, model_name, task_name, work_deriv, log_dir
):
    """Title."""
    #
    for sess in sess_list:
        if sess not in ["ses-day2", "ses-day3"]:
            raise ValueError(f"Unexpected session : {sess}")
    ...
    for proc in mult_proc:
        proc.join()
    print("Done : workflow._mine_res4d", flush=True)
